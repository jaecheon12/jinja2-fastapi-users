...
def select_model(request: Request, code: str = Form(...), sqldb: Session = Depends(get_db)):
    try:
        res_makers = CtrlMaker(sqldb).select_all(code)
        res_media = CtrlMedia(sqldb).selectCode(code)
        ...
        for maker in res_makers:
            if maker.MakerId:
                if maker.MakerId[0:4] == 'M000':
                    res_model = ctrl_model.get_bycode(maker.MakerId)
                    if res_model:
